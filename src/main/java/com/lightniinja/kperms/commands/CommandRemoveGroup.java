...
public class CommandRemoveGroup {
	...
	public CommandRemoveGroup(CommandSender s, KPermsPlugin pl, String player, String group) {
		this.s = s;
		this.pl = pl;
	...
	public void execute() {
		if(!this.s.hasPermission("kperms.user.group.remove")) {
		...
		if(p.removeGroup(this.group)) {
			p.clearPermissions();
			p.assignPermissions();
			String str = this.u.format(this.m.getMessage("prefix") + " " + this.m.getMessage("removed-group"));
